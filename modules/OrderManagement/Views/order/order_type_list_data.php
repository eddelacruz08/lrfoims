        ...
        <?php if(user_link('orders/preparing', session()->get('userPermissionView'))):?>
            <?php if($typeInfo['order_status_id'] == 2):?>
                <div class="list-group border mb-1" id="borderStyle<?=$typeInfo['id']?>">
                    <a type="button" onclick="displayOrderTypeInfo('<?=base_url()?>/orders/get-info',<?=$typeInfo['id']?>,<?=$typeInfo['number']?> ,<?=$orderMaxLimit['max_limit']?>);" class="list-group-item list-group-item-action">
