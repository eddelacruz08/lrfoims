                            ...
                            <?php foreach ($menuLists as $row) : ?>
                                <?php if($category['id'] == $row['menu_category_id']): ?>
                                    <div class="col-sm-12 col-md-6 col-lg-3 col-xxl-3 special-grid <?=$category['id']?>" id="resize-card-contents">
                                        <div type="button" onclick="addToCartAdminMenu('/orders/admin-menu/add-to-cart/a', <?=$row['id']?>, <?=$orderDetails['id'];?>,'/orders/admin-menu/order-cart-list-data');" class="card h-70 rounded shadow p-1" id="menu-list-hover">
