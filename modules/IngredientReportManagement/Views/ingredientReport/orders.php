...
function orderTypeDineInList(){
  ...
  $.ajax({
    url: "<?=base_url()?>/orders/order-type-dine-in-list-data/1",
    type: 'GET',
    data: {},
...
function orderTypeTakeOutList(){
  ...
  $.ajax({
    url: "<?=base_url()?>/orders/order-type-take-out-list-data/2",
    type: 'GET',
    data: {},
    success: function (html) {
      element.html(html);
...
function orderTypeDeliveryList(){
  ...
  $.ajax({
    url: "<?=base_url()?>/orders/order-type-delivery-list-data/3",
    type: 'GET',
    data: {},
    success: function (html) {
      element.html(html);
...
function getMenuList(route, id, orderNumber, orderLimit, url){
	$.ajax({
		type: "GET",
		url: '<?=base_url()?>/menu-list/get-menu-list/',
		async: true,
		dataType: 'JSON',
      ...
      for (let i = 0; i < response.menu.length; ++i) {
        if(response.menu[i]['menu_status'] === 'a'){
          var html = '<div class="border p-3 mb-1 rounded" id="menu_id_border">';
              ...
              html += '          <img src="<?=base_url()?>/assets/uploads/menu/' + response.menu[i]['image'] + '" class="menu-image-style rounded" alt="...">';
              html += '       </div>';
              html += '     </div>';
              html += '   </div>';
              html += ' </div>';
          ...
          options = options.concat(html);
			...
			Swal.fire({
              ...
              '       <input type="number" id="quantity" value="1" min="1" max="10" name="quantity" onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57" placeholder="Enter quantity . . ." required class="form-control is-valid"/>'+
              '     </div>'+
              '   </div>'+
              ' </div>'+
              '</div>',
